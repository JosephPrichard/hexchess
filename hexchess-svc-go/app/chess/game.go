...
func StartGame() Game {
	return Game{Board: InitialBoard()}
}

func EmptyGame() Game {
	return Game{Board: MakeBoard(true)}
}
...
func (g *Game) InitPieceMoves() {
	...
	whiteKingMoves := PieceMoves{
		From:  whiteKingHex,
		Moves: g.FindKingMoves(whiteKingHex),
	}
	blackKingMoves := PieceMoves{
		From:  blackKingHex,
		Moves: g.FindKingMoves(blackKingHex),
	}
...
func (g *Game) FindKingMoves(hex Hexagon) []Hexagon {
	// we want all moves, so nothing is attacking
	return g.FindKingMovesFiltered(hex, func(h Hexagon) bool { return true })
}
