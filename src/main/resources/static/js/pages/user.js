function createGetReplays(userId) {
    ...
    return async () => {
        const tableElement = document.getElementById('replay-table-tbody');

        const isAtPageBottom = window.innerHeight + window.scrollY >= document.body.offsetHeight;
        if (!hasMoreRecords || !isAtPageBottom || !tableElement.lastElementChild) {
            return;
        }

        const lastId = tableElement.lastElementChild.getAttribute("data-id");
...
async function attachEventListeners(userId) {
