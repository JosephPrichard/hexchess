...
import { nextGameInput } from "k6/x/hexchess/websocket";
...
function newTrendWithCount(name) {
    return {
...
export const trendMoveIntoMove = newTrendWithCount("move_into_ok_responses");
export const trendMoveIntoErrors = newTrendWithCount("move_into_errors");
...
export default async function (data) {
    ...
    for (let i = 0; i < playersCount; i++) {
        ...
        tempSessionIds.push(JSON.parse(sessResp.body).sessionId)
    }

    const messageMetrics = {};

    // excute: play by responding to output from server until game is terminal
    const gameConnections = tempSessionIds.map((sessionId) => connectGame(messageMetrics, gameId, sessionId));
    ...
    // note: this happens in a closure because k6 doesn't allow blocking with promise all.
    for (const [_, metric] of Object.entries(messageMetrics)) {
...
const connectGame = (messageMetrics, gameId, sessionId) => {
    ...
    return new Promise((resolve) => {
        ...
        socket.addEventListener("message", (event) => {
            // on output from server, prepare to send an input back
            const outputBytes = event.data;
            const inputMessageId = uuidv4();

            messageMetrics[inputMessageId] = {inputTime: new Date(), inputType: "move", outputs: []};
            ...
            if (outputMessageId) {
                const metric = messageMetrics[outputMessageId];
                if (!metric) {
