...
export function setupSessions(usersCount: number): Session[] {
    ...
    return sessionResps.map(resp => {
        if (resp.status != 200) {
            throw new Error("!2xx resonse while setting up sessions");
        }
        return { 
...
export function setupGames(gamesCount: number): string[] {
    ...
    return gameResps.map(resp => {
        if (resp.status != 200) {
            throw new Error("!2xx resonse while setting up games");
        }
        return String(JSON.parse(resp.body as string).gameId);
    });
}