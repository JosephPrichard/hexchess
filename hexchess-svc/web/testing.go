...
func createTestChessStates(t *testing.T, rdb *data.Redis) {
	...
	state1 := data.MakeState("game1", data.TcRealTime, data.CsRandom, nil)
	state1.WhitePlayer = &data.PlayerState{ID: 2, Name: "user2", Country: "us", Elo: 1000}

	for _, state := range []data.ChessState{
		state1,
		data.MakeState("game2", data.TcRealTime, data.CsRandom, nil),
		data.MakeState("game3", data.TcRealTime, data.CsRandom, nil),
	} {
