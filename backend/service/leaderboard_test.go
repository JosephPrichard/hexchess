...
func setupLeaderboardTest(t slogutil.TestLogger) (*LeaderboardService, itest.TestInfra) {
...
func TestLeaderboard(t *testing.T) {
	services, testinfra := setupLeaderboardTest(t)
...
func TestGetFullLeaderboardUsers(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantLeaderboard: []model.LbdUser{
				{
					PendingUser: model.User{ID: 1, Username: "user1", Country: "us", JoinedOn: itest.TimeNow},
					Elo:         1050,
					HighestElo:  1050,
					Wins:        6,
					Losses:      5,
		...
		},
		{
			...
			wantLeaderboard: []model.LbdUser{
				...
				{
					PendingUser: model.User{ID: 3, Username: "user3", Country: "us", JoinedOn: itest.TimeNow},
					Elo:         900,
					HighestElo:  900,
					Rank:        2,
				},
...
func TestGetFuzzySearchLeaderboard(t *testing.T) {
	...
	wantUsers := []model.LbdUser{
		{
			PendingUser: model.User{ID: 8, Username: "john", Country: "us", Bio: "", JoinedOn: time.Date(1, time.January, 1, 0, 0, 0, 0, time.UTC)},
			Elo:         1500,
			HighestElo:  2000,
			Wins:        12,
			Losses:      4,
		...
		{
			PendingUser: model.User{ID: 9, Username: "johnny", Country: "us", Bio: "", JoinedOn: time.Date(1, time.January, 1, 0, 0, 0, 0, time.UTC)},
			Elo:         1500,
			HighestElo:  1500,
			Wins:        5,
			Losses:      2,
